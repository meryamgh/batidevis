...
const FullQuote: React.FC = () => {
  ...
  const tvaRate = 0.20; 
  const [acompteRate, setAcompteRate] = useState(0.30);
  ...
  // Gestion de l'édition du pourcentage d'acompte
  const [editingAcompte, setEditingAcompte] = useState(false);
  const [acompteEditValue, setAcompteEditValue] = useState((acompteRate * 100).toString());
  ...
  const saveAcompteChanges = () => {
    let val = parseFloat(acompteEditValue);
    if (isNaN(val) || val < 0) {
      val = 30; // Valeur par défaut si l'utilisateur saisit quelque chose d'invalide
    }
    // On stocke la valeur en tant que fraction : exemple: 30% -> 0.30
